'''An IoU calculator and a class to encode and decode targets for the SSD model.'''
...
def decode_y(y_pred):
    ...
    y_pred_classes_decoded = (np.copy(y_pred[:,:,-5:])).astype(np.int32)
    y_pred_classes_decoded[:,:,0] = np.argmax(y_pred[:,:,:-4], axis=-1)
...
class SSDBoxEncoder:
    ...
    In the process of encoding ground truth labels, a template of default boxes
    ...
    def __init__(self,
        ...
        Arguments:
            ...
            min_scale (float, optional): The smallest scaling factor for the size of the default boxes as a fraction
                of the shorter side of the input images. Defaults to 0.1.
            max_scale (float, optional): The largest scaling factor for the size of the default boxes as a fraction
            ...
            iou_threshold (float, optional): The intersection-over-union similarity threshold that must be
                met in order to match a given ground truth box to a given default box. Defaults to 0.5.
        '''
    ...
    def build_default_boxes(self,
                            ...
                            diagnostics=False):
        '''
        Compute an array of the spatial positions and sizes of the default boxes for one particular classification
        layer of size `feature_map_size == [feature_map_height, feature_map_width]`.

        Returns a 4D Numpy tensor of shape `(feature_map_height, feature_map_width, n_boxes_per_cell, 4)` where the
        last dimension contains `(xmin, xmax, ymin, ymax)` for each default box in each cell of the feature map.
        '''
    ...
    def build_y_encode_template(self, batch_size, diagnostics=False):
        '''
        ...
        yourself aware of this fact and why it is necessary in order to understand how this network architecture works.

        In other words, the boxes in `y_encoded` must have a specific order in order correspond to the right spatial
        positions and scales of the boxes predicted by the model. This sequence of operations ensures that `y_encoded`
        has this specific form.
        '''

        # 1: Get the default box scaling factors for each conv layer from which we're going to make predictions
        ...
        if diagnostics:
            ...
            for i in range(len(self.scales)-1):
                boxes, wh, cells = self.build_default_boxes(batch_size=batch_size,
            ...
            for i in range(len(self.scales)-1):
                boxes_tensor.append(self.build_default_boxes(batch_size=batch_size,
                                                             feature_map_size=self.classifier_sizes[i],
                                                             this_scale=self.scales[i],
                                                             next_scale=self.scales[i+1],
                                                             diagnostics=False))
        ...
        # 4: Concatenate the two tensors for classes and boxes to get our final template for y_encoded
    ...
    def encode_y(self, ground_truth_labels):
        ...
        default bounding box in a template with respect to their jaccard similarity. If the jaccard similarity is greater than
        or equal to the set threshold, the boxes will be matched, meaning that the ground truth box coordinates and class
        will be written to the the specific position of the matched default box in the template.

        The class for all default boxes for which there was no match with any ground truth box will be set to the
        background class.
        ...
        # 1: Build the template for y_encoded
        y_encode_template = self.build_y_encode_template(batch_size=len(ground_truth_labels),
                                                         diagnostics=False)

        y_encoded = np.copy(y_encode_template) # We'll write the ground truth box data to this array

        # 2: Match the boxes from `ground_truth_labels` to the default boxes in `y_encode_template`
        #    and for each matched box record the ground truth coordinates in `y_encoded`.
        #    Every time there is no match for a default box, record `class_id` 0 in `y_encoded` for that default box.
        #    TODO: This matching process is slow. Make it faster, avoid loops where possible.

        class_vector = np.eye(self.n_classes, dtype=np.int32) # An identity matrix that we'll use as one-hot class vectors

        for i in range(len(y_encode_template)):
            available_boxes = np.ones((y_encode_template.shape[1]), dtype=np.int32) # 1 for all default boxes that are not yet matched to a ground truth box, 0 otherwise
            for true_box in ground_truth_labels[i]:
                similarities = iou(y_encode_template[i,:,-4:], true_box[:4]) # The iou similarities for all default boxes
                similarities *= available_boxes # Filter out default boxes which aren't available anymore (i.e. already matched to a different ground truth box)
                available_and_thresh_met = np.copy(similarities)
                available_and_thresh_met[available_and_thresh_met < self.iou_threshold] = 0 # Filter out default boxes which don't meet the iou threshold
                assign_indices = np.nonzero(available_and_thresh_met)[0] # Get the indices of the left-over default boxes to which we want to assign this ground truth box
                if len(assign_indices) > 0: # If we have any matches
                    y_encoded[i,assign_indices,:] = np.concatenate((class_vector[true_box[4]], true_box[:4].astype(np.int32)), axis=0) # Write the ground truth box coordinates and class to all assigned default box positions
                    available_boxes[assign_indices] = 0 # Make the assigned default boxes unavailable for the next ground truth box
                    ...
                    y_encoded[i,best_match_index,:] = np.concatenate((class_vector[true_box[4]], true_box[:4].astype(np.int32)), axis=0) # Write the ground truth box coordinates and class to the best match default box position
                    available_boxes[best_match_index] = 0 # Make the assigned default box unavailable for the next ground truth box
            # Set the classes of all remaining available default boxes to class zero
            background_class_indices = np.nonzero(available_boxes)[0]
            y_encoded[i,background_class_indices,0] = 1

        return y_encoded

    def build_loc_norm_tensor(self, batch_size):
        '''
        Generate a Numpy array containing the width and height of each default box.

        This array corresponds to the structure for the targets required by the SSD loss function
        and is used to normalize the localization loss.

        Returns:
            A Numpy array of shape `(batch_size, #boxes, 4)` where `#boxes` is the total number of
            default boxes per image and the last dimension contains `(width, width, height, height)`
        ...
        # 2: For each conv classifier layer (i.e. for each scale factor) get the tensors for
        #    the box coordinates of shape `(batch, n_boxes, 4)`
        boxes_tensor = []
        for i in range(len(self.scales)-1):
            boxes_tensor.append(self.build_default_boxes(batch_size=batch_size,
                                                         feature_map_size=self.classifier_sizes[i],
        ...
        # 3: Concatenate all boxes to one tensor of shape `(batch, n_boxes_total, 4)`
        boxes_tensor = np.concatenate(boxes_tensor, axis=1)

        # 4: Replace xmin and xmax with the box width, replace ymin and ymax with the box height
        #    Why? Our cost function is going to use this tensor to normalize the localization loss
        #    by dividing horizontal deviations from the ground truth by the default box width, and
        #    by dividing vertical deviations from the ground truth by the default box height.
        boxes_tensor[:,:,[0,1]] = np.expand_dims(boxes_tensor[:,:,1] - boxes_tensor[:,:,0], axis=-1)
        boxes_tensor[:,:,[2,3]] = np.expand_dims(boxes_tensor[:,:,3] - boxes_tensor[:,:,2], axis=-1)
