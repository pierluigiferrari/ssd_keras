...
def build_filelists(path='./data/labels.csv', n_classes=3):
    ...
    The input CSV file is expected to list each ground truth bounding box for a given image in a separate
    line. The lines pertaining to the same image do not necessarily need to be consecutive.

    Arguments:
        path (string, optional): The filepath to a CSV file that contains lines with
            `(relative image file name, xmin, xmax, ymin, ymax, class_id)` for each ground truth bounding box.
        n_classes (int, optional): The number of classes to include. Set to `False` to ignore.

    Returns:
        Returns two python lists where the first list contains the filepath to each image and the second list
        contains one numpy array per image, where each numpy array contains the coordinates
        (xmin, xmax, ymin, ymax, class_id) for all ground truth bounding boxes belonging to a given image.
    '''
    ...
    filenames = [] # All unique image filenames will go here
    labels = [] # Each entry here will contain a 2D numpy array with all the ground truth boxes for a given image
...
def generate_batch(filenames,
    ...
    Arguments:
        filenames (array-like): A list or 1D numpy array containing all relative file paths
            to the samples from which the batches are to be generated. Note that
            `filenames` must not contain the actual files themselves.
        labels (array-like): A list containing one 2D numpy array per image. For an image with `k`
            ground truth bounding boxes, the respective 2D array has `k` rows, each row containing
            `(xmin, xmax, ymin, ymax, class_id)` for the respective bounding box.
        path_prefix (string, optional): A string containing a directory path to be prepended to each filename
            in `filenames`.
    ...
    filenames, labels = shuffle(filenames, labels) # Shuffle the data before we begin
    current = 0
    ...
    while True:
        ...
        for filename in filenames[current:current+batch_size]:
            with Image.open('{}{}'.format(path_prefix, filename)) as img:
                batch_X.append(np.array(img))
        batch_y = deepcopy(labels[current:current+batch_size])

        if diagnostics:
            ret1 = filenames[current:current+batch_size] # The filenames of the files in the batch
        ...
        for i in range(len(batch_X)):
            ...
            # usefulnees of vertical flipping either empirically or theoretically, so I'm going for simplicity.
...
def offline_image_processing(filenames,
                             labels,
                             source_path_prefix='',
                             dest_path_prefix='',
                             start=0,
    ...
    Arguments:
        ...
        labels (array-like): A list containing one 2D numpy array per image. For an image with `k`
            ground truth bounding boxes, the respective 2D array has `k` rows, each row containing
            `(xmin, xmax, ymin, ymax, class_id)` for the respective bounding box.
        source_path_prefix (string, optional): A string containing a directory path to be prepended to each filename
            in `filenames`.
        dest_path_prefix (string, optional): The destination directory where the processed images
            and `labels.csv` should be saved.
    ...
    for k, filename in enumerate(filenames[start:stop]):
        i = k + start
        with Image.open('{}{}'.format(source_path_prefix, filename)) as img:
            image = np.array(img)
        targets = np.copy(labels[i])
        ...
        img.save('{}{}'.format(dest_path_prefix, filename), 'JPEG', quality=90)
        del image
        del img
        gc.collect()
    ...
    with open('{}labels.csv'.format(dest_path_prefix), 'w', newline='') as csvfile:
